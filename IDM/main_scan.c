...
{ int err;
  ...
  //double dmC; 
  //double l;
  //double l[4] = {1e-3,1e-2,1e-1,1e0}; //test 
  int n,i,j,k,o;
  char buffer[50];
  ...
  for (i = 0; i <= 250000; i+=1){
  //for (Mdm = 300.0; Mdm <= 30000.0; Mdm*= 1.10){
  //for (o = 0; o <= 50; o+=1){
  double Mdm = 300*pow(10,2*(double)rand()/(double)RAND_MAX);
  //for (i = 0; i <= 50; i+=1){
  double lamb = pow(10,-2.0 +2.792392*(double)rand()/(double)RAND_MAX); // 
  //for (j = 0; j <= 25; j+=1){
  double dm0 = 0.5*pow(10,1.30103*(double)rand()/(double)RAND_MAX);
  //for (k = 0; k <= 25; k+=1){
  double dmC = 0.5*pow(10,1.30103*(double)rand()/(double)RAND_MAX);
  
  //double lamb = pow(10,-2.0 +(0.33377+1.484884*log10(Mdm/300.0))*(double)rand()/(double)RAND_MAX); // experimental XENONnT limit here
  //for (j = 0; j <= 1; j+=1){ 				// dm0 range 
  //for (i = 0; i <= 1; i+=1){				// dm+ range
  //for (lamb = 0.01; lamb <= 6.2; lamb*= 1.03){ 				// lambda range
  		// Mdm range test
...
//   VZdecay=2; VWdecay=2; cleanDecayTable(); 
  //if (log10(lamb)<=-1.666230+1.484884*log10(Mdm/300.0)) // XENONnT limit
  //{    
  printf("\n==== Calculation of relic density =====\n"); 
  Omega=darkOmega(&Xf,fast,Beps,&err);
  //sigma = vSigma(Mdm/Xf,Beps,fast);
  fprintf(Sigma,"%.4E %.4E %.4E %.4E ",lamb, Mdm, dm0, dmC);
  fprintf(Sigma,"%.4E\n",Omega);  
...
{ double pA0[2],pA5[2],nA0[2],nA5[2];
  ...
  printf("\n==== Calculation of CDM-nucleons amplitudes  =====\n");   
    ...
    csSIp=  SCcoeff*pA0[0]*pA0[0]; // imprimir na tabela 
...
{ char* expName; 
  ...
  // colocar 0 if excluido  
  // colocar seção de choque independente de spin
  //if(pval<0.1 ) DD = 0; //printf("Excluded by %s  %.1f%% \n", expName, 100*(1-pval)); 
  //else DD = 1;//printf("Not excluded by DD experiments  at 90%% level \n"); 
  //fprintf(Sigma,"%d\n", DD);  
}
