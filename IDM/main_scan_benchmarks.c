...
{ int err;
  double Mdm, M2, M3, lamb;
  double dm0[4] = {0.5,1.0,5.0,10.0};
  double dmC[4] = {0.5,1.0,5.0,10.0}; 
  //double l;
  ...
  n=sprintf(buffer,"IDM_benchmarks");
  FILE* Sigma = fopen(buffer,"w");
  
  fprintf(Sigma,"Lambda345 MDM d0 d+ Omega \n");
  
  for (j = 0; j <= 3; j+=1){ 				// dm0 range 
  for (i = 0; i <= 3; i+=1){				// dm+ range
  for (lamb = 0.01; lamb <= 6.2; lamb*= 1.03){ 				// lambda range
  for (Mdm = 300.0; Mdm <= 30000.0; Mdm*= 1.02){		// Mdm range test
  ...
  assignVal("MHC",M2);
  assignVal("MH3",M3);
  fprintf(Sigma,"%.4E %.4E %.4E %.4E ",lamb, Mdm, dm0[j], dmC[i]);   
